...
import { Calendar as CalendarIcon, MapPin, ExternalLink, Trophy, Plus, X, Trash2, Edit3, Map } from 'lucide-react';
...
const CalendarioProvas: React.FC<CalendarioProvasProps> = ({ races, onAddRace, onUpdateRace, onDeleteRace }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Fix: Removed incorrect reference to 'editingGoal' which was a copy-paste error from another component.
    if (editingRace) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
           ...
           <p className="text-slate-500 text-xs">Mantenha seu calendário de competições atualizado.</p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-white transition-all shadow-lg shadow-emerald-900/20"
        >
      ...
      <div className="grid grid-cols-1 gap-4">
        {races.length > 0 ? races.map(race => (
          <div key={race.id} className="glass p-5 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-4 border border-white/5 hover:border-emerald-500/30 transition-all group">
            <div className="flex items-center gap-4">
              <div className="text-center w-14 py-2 px-1 bg-slate-900/80 rounded-2xl border border-white/5 shadow-inner">
                ...
                <p className="text-xl font-black leading-none text-emerald-500">{new Date(race.date).getDate() + 1}</p>
              </div>
              <div>
          ...
          <div className="py-20 text-center glass rounded-[3rem] border-dashed border-2 border-slate-800">
             ...
             <p className="text-slate-600 text-xs mt-2">Adicione seu próximo desafio ao calendário.</p>
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 lg:p-8 space-y-5">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  placeholder="Ex: Maratona de Chicago"
